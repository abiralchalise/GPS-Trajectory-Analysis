...
→ Open: [GPS_data_analysis.ipynb](GPS_data_analysis.ipynb)
...
- QGIS required for spatial steps (can be skipped for analysis/visualization focus)  
- Minor dtype warnings can be fixed with explicit dtypes in `pd.read_csv`

This project shows careful statistical justification before deriving planning-level metrics — I believe this level of rigor is important for reliable transportation analysis.

I am happy to explain any part in more detail, provide sample outputs, or discuss the methodology further.
